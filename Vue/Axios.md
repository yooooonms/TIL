...
서버와 데이터를 송수신 할 수 있는 HTTP 비동기 통신 라이브러리로 Vue.js 개발 시 서버와 통신을 위해 많이 사용되는 라이브러리이다.

Promise 객체 형태로 값을 반환해 .then()으로 결과값을 받아서 처리하는 형식으로 구성되어 있다.
...
메소드 별칭을 사용하면 url, method, data 속성을 지정할 필요가 없다. 제공되는 메소드 별칭은 다음과 같다.
...
create() 메소드를 사용해 사용자 정의 구성을 사용하는 axios 인스턴스를 생성할 수 있다.
...
사용 가능한 config 옵션 url 속성은 필수로 작성해야 하고, method가 지정되지 않으면 get으로 기본 설정되고 나머지 속성은 옵션이다.
